...
const no_weapon_config_dictionary = {
    "selectedWeaponHand":HAND_NONE, 
...
function equip_inner(curLine, curName, curSource, leftID, rightID, fistsID, selectedWeaponHand, type) {
    // Unequip 
    let ued = {};
    let actionString = "";
    if( curSource == leftID && curLine != 0) {
        ...
        if ( selectedWeaponHand == HAND_LEFT ) {
            Object.assign(ued, no_weapon_config_dictionary);
        }
    }
    if( curSource == rightID && curLine != 1) {
        ...
        if ( selectedWeaponHand == HAND_RIGHT ) {
            Object.assign(ued, no_weapon_config_dictionary);
        }
    }
        ...
        generate_unequip_dictionary(
            ...
            async (dictionary) => {
                ...
                if(curLine == 0) {
                    ...
                    if ( selectedWeaponHand == HAND_LEFT ) {
                        Object.assign(dictionary, no_weapon_config_dictionary);
                    }
                } else if (curLine == 1) {
...
on("change:repeating_weaponsranged:wr_line", async (eventInfo) => {
    const v1 = getAttrs(["repeating_weaponsranged_wr_line", "repeating_weaponsranged_wr_name", "inv_hand_left_id", "inv_hand_right_id", "global_fists_id", "selectedWeaponHand"], (v1) => {
 ...
 on("change:repeating_weaponsmelee:wm_line", async (eventInfo) => {
    const v1 = getAttrs(["repeating_weaponsmelee_wm_line", "repeating_weaponsmelee_wm_name", "inv_hand_left_id", "inv_hand_right_id", "global_fists_id", "selectedWeaponHand"], (v1) => {
        ...
        let fistsID = v1.global_fists_id;
        let selectedWeaponHand = v1.selectedWeaponHand;
        equip_inner(curLine, curName, curSource, leftID, rightID, fistsID, selectedWeaponHand, WEAPON_TYPE_MELEE);
    });
 });
...
function setWeaponSkillsSheet(hand) {
    ...
    let dictionary = {};
    Object.assign(dictionary, no_weapon_config_dictionary);
    dictionary["selectedWeaponHand"] = hand;
